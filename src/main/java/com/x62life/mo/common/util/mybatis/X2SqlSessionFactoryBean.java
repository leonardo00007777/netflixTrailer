...
public class X2SqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>, InitializingBean,
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(X2SqlSessionFactoryBean.class);
    ...
    protected SqlSessionFactory buildSqlSessionFactory() throws Exception {
        XMLConfigBuilder xmlConfigBuilder = this.configLocation == null
                ? null
                : new XMLConfigBuilder(this.configLocation.getInputStream(), null, this.configurationProperties);

        Configuration configuration = initConfiguration(xmlConfigBuilder);
        ...
        try {
            ...
            registerConfigurationEnvironment(configuration);
            registerDatabaseId(configuration);
            registerMappers(configuration);
        } catch (Exception e) {
            LOGGER.debug("Configuration Failed: {}", e);
            // ignored...
        }
        return this.sqlSessionFactoryBuilder.build(configuration);
    }

    private void registerDatabaseId(Configuration configuration) throws NestedIOException {
    ...
    private void registerConfigurationEnvironment(Configuration configuration) {
    ...
    private void registerMappers(Configuration configuration) throws Exception {
        if (!isEmpty(this.mapperLocations)) {
            for (Resource mapperLocation : this.mapperLocations) {
                ...
                try {
                    ...
                    final String pattern = "Failed to parse mapping resource: {0} \n===>{1}";
                    final String exceptionMessage = MessageFormat.format(pattern, mapperLocation, e);
    ...
    private void parseXmlConfig(XMLConfigBuilder xmlConfigBuilder) throws NestedIOException {
        try {
            ...
            final String pattern = "Failed to parse config resource: {0} \n===> {1}";

            final String exceptionMessage = MessageFormat.format(pattern, this.configLocation.toString(),
                    ex.getMessage());
            LOGGER.error(exceptionMessage);

            throw new NestedIOException(exceptionMessage, ex);
        } finally {
            ErrorContext.instance().reset();
        }
    }
    ...
    private void registerPlugins(Configuration configuration) {
    ...
    private void registerAliases(Configuration configuration) {
    ...
    private Configuration initConfiguration(XMLConfigBuilder xmlConfigBuilder) {
