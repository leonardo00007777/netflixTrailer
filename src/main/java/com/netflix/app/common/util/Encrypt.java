...
public class Encrypt 
{
    ...
    static final String pEncKey = "SAMSUNG_SDS_CANADA_ENCRYPTOR_v_1.1";
    ...
    {
        into = "";
        into = (new StringBuilder(String.valueOf(into))).append((char)(int)(outof[0] >> 24 & 255L)).toString();
        into = (new StringBuilder(String.valueOf(into))).append((char)(int)(outof[0] >> 16 & 255L)).toString();
        into = (new StringBuilder(String.valueOf(into))).append((char)(int)(outof[0] >> 8 & 255L)).toString();
        into = (new StringBuilder(String.valueOf(into))).append((char)(int)(outof[0] & 255L)).toString();
        into = (new StringBuilder(String.valueOf(into))).append((char)(int)(outof[1] >> 24 & 255L)).toString();
        into = (new StringBuilder(String.valueOf(into))).append((char)(int)(outof[1] >> 16 & 255L)).toString();
        into = (new StringBuilder(String.valueOf(into))).append((char)(int)(outof[1] >> 8 & 255L)).toString();
        into = (new StringBuilder(String.valueOf(into))).append((char)(int)(outof[1] & 255L)).toString();
        return into;
    }
    ...
    static String des_dec(String dat, int blocks)
    {
        ...
        for(int i = 0; i < blocks; i++)
        {
            ...
            share--;
            resStr = (new StringBuilder(String.valueOf(resStr))).append(myStr).toString();
        }
    ...
    public static String com_EncodeJtoJ(String pPlainText)
    {
        ...
        int iCTLen = 0;
        int iBlkSize = 0;
        pPlainText = (new StringBuilder(String.valueOf(pPlainText))).append("|").toString();
        pPlainText = Ko2En(pPlainText);
        iCTLen = pPlainText.length() + 8;
        iBlkSize = (iCTLen - 1) / 8 + 1;
        iCTLen = iBlkSize * 8 + 1;
        pCT = pPlainText;
        DES_SetKey("SAMSUNG_SDS_CANADA_ENCRYPTOR_v_1.1");
        ...
        for(int i = 0; i < iBlkSize * 2 - pCTHexNo; i++)
            pCTHex = (new StringBuilder(String.valueOf(pCTHex))).append("0").toString();
    ...
    static String HexToStrBlk(String pHex)
    {
        ...
        for(i = 0; i < SrcLen; i++)
        {
            ...
            if(pHex.charAt(i) >= '0' && pHex.charAt(i) <= '9')
                ...
                hv += (pHex.charAt(i) - 97) + 10;
            pStr = (new StringBuilder(String.valueOf(pStr))).append((char)hv).toString();
        }
    ...
    {
        String strRet = null;
        strRet = com_Encode((new StringBuilder(":")).append(strData).append(":sisenc").toString());
        return strRet;
    }
    ...
    public static boolean compareMD5Pwd(String target, String md5Encoded)
    {
        return md5Encoded.equals(MD5_encode(target));
    }
