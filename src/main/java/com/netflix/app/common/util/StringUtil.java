...
import com.netflix.app.model.common.Code;
...
//        value = scriptPattern.matcher(value).replaceAll("<x-embed");
	 ...
	 * 2. ClassName		: StringUtil
	 * 3. Commnet		: 입력된 문자열의 오른쪽에 index의 길이만큼 문자열을 붙여 반환한다.(Byte로 계산)
	 * 4. 작성자			: LeeJongHo
	 * 5. 작성일			: 2020. 5. 28. 오후 6:18:20
	 * @return String
	 * @param str
	 * @param index
	 * @param addStr
	 ...
	 * 2. ClassName		: StringUtil
	 * 3. Commnet		: 입력된 문자열의 Byte Length를 반환한다.
	 * 4. 작성자			: LeeJongHo
	 * 5. 작성일			: 2020. 5. 28.. 오후 6:03:43
	 * @param str
	 * @return
	 */
    ...
    /**
     * 1. MethodName	: substrByte
     * 2. ClassName		: StringUtil.java
     * 3. Commnet		: 한글을 2바이트로 인식하여 반환한다.(substr 처럼 사용하면 됨)
     * 4. 작성자			: LeeJongHo
     * 5. 작성일			: 2020. 5. 28. 오후 6:08:58
     * @return String
     * @param str
     * @param iStartPos
     * @param iByteLength
     * @return
     */
    public static String substrByte(String str, int iStartPos, int iByteLength) {
    	byte[] bytTemp = null;
		int iRealStart = 0;
		int iRealEnd = 0;
		int iLength = 0;
		...
		try {
			...
			iRealEnd = iRealStart;
			int iEndLength = iStartPos + iByteLength;
			for(int iIndex = iStartPos; iIndex < iLength && iIndex < iEndLength; iIndex++)
			{
				iChar = (int)bytTemp[iIndex];
				if((iChar > 127)|| (iChar < 0)) {
					// 한글의 경우(2byte 통과처리)
		...
		return str.substring(iRealStart, iRealEnd);
    }
    
    /**
     * 1. MethodName	: getCodeValue
     * 2. ClassName		: StringUtil.java
     * 3. Commnet		: Code.java에 있는 int 값을 불러온다.
     ...
     * @return
     */
    public static int getCodeIndexValue(String str) {
    	int retVal = 0;
        try{
     	   Field f = Code.class.getField(str);
     	   retVal = f.getInt(null);
     ...
     * @see
     */    
    public static String getEditerHtmlOut(String srcString) {
    	
    	String tmpString = srcString;
    	
		try 
		{
			tmpString = tmpString.replaceAll("<(/)?([a-zA-Z]*)(\\s[a-zA-Z]*=[^>]*)?(\\s)*(/)?>", "");
		}
		catch (Exception ex)
		{
			//Logger.getLogger(StringUtil.class).debug(ex);//ex.printStackTrace();
	 ...
	 * @see
	 */    
    public static String filterText(String banTxt , String sText) {
    	
    	String returnTxt = "";
        Pattern p = Pattern.compile(banTxt, Pattern.CASE_INSENSITIVE);
        ...
        if(sb.toString().indexOf(',') > 0){
        	returnTxt = sb.substring(0, (sb.length()) - 1);
        }else{
        	returnTxt = sb.toString();
        }
        return returnTxt;
    }
    ...
    /**
     * <pre>
     * 1. MethodName : textSumDelimiter
     * 2. ClassName  : StringUtil.java
     * 3. Comment    : 문자에 특정문자 붙히기.
	...
	public static String textSumDelimiter(StringBuffer txt) {
