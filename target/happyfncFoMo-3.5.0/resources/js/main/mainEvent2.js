...
mmain.main = {
    ...
    mainGnbSwiper : null,
    /* 마지막 탭 index 가져옴. mmain.main.makeSwipeHtml, mmain.main.setGnbSwipe 마지막이 5로 하드코딩 되어 있던 부분 수정.*/
    gnbLastIdx : $("#mGnb").find(".gnb_cate li").length - 1, 
...
//                    sessionStorage.clear();
        ...
        $("#mGnb").find("li").each(function(idx) {
            $("#mGnb").find("li").eq(idx).find("a").bind("click", function() {
                $('#main-swiper-tab0 .mVisual-slide').ixOverlayList('clear');
...
//                             }
                        ...
                        if(mmain.best.interval != null) {
                        	clearInterval(mmain.best.interval);
                        }
            });
        });
...
//                mmain.menu.moveSubMenu(gnbTabIdx,  $("#mGnb").find("li").eq((gnbTabIdx)).attr("data-ref-url"));
        ...
        $(".crtPopFullWrap").find(".btnClose").bind("click", function() {
...
//        $("#mGnb").find("li").eq(prev).attr("data-ref-name")
        ...
        // 왼쪽화면
        var isPrev = false;
        let prev = gnbTabIdx <= 0 ? mmain.main.gnbLastIdx : gnbTabIdx - 1;
        ...
        // 오른쪽화면
        var isNext = false;
        let next = gnbTabIdx >= mmain.main.gnbLastIdx ? 0 : gnbTabIdx + 1;
        ...
        if(isMain){
        	...
        	$('html > head > title').text($("#mGnb").find("li").eq(pos).find("a > span").text() + ' | 올리브영');
            setTimeout(function() {
                try {
                    ...
                    var date 	= new Date();
                    var today 	= "" + date.getFullYear() + ("0" + (1 + date.getMonth())).slice(-2) + ("0" + date.getDate()).slice(-2)    + ("0" + date.getHours()).slice(-2);
                    if (today >= '2021100709' && today <= '2021100823') {
                        switch (pos) {
                            case 0: //홈
                                common.wlog('home_gnb_home');
                                break;
                            case 1: //메가브랜드-라로슈포제
                                common.wlog('home_gnb_espoir');
                                break;
                            case 2: //오특
    ...
    setGnbSwipe : function(init_gnb_tab_idx) {
    	...
    	if(init_gnb_tab_idx != '0'){
    		mmain.main.closeFirstPurchasePop('firstPurchase_sticky');
    	...
    	/* megabrand
        switch (init_gnb_tab_idx) {
            case '1': common.wlog('home_gnb_hotdeal');  break;
            case '2': common.wlog('home_gnb_new');      break;
            case '3': common.wlog('home_gnb_best');     break;
            case '4': common.wlog('home_gnb_planshop'); break;
            case '5': common.wlog('home_gnb_event');    break;
            case '6': common.wlog('gnb_foru');          break;
        }
        */
        var date	= new Date();
        var today 	= "" + date.getFullYear() + ("0" + (1 + date.getMonth())).slice(-2) + ("0" + date.getDate()).slice(-2)    + ("0" + date.getHours()).slice(-2);
        if (today >= '2021100709' && today <= '2021100823') {
        	switch (init_gnb_tab_idx) {
		        case '1': common.wlog('home_gnb_hotdeal');  break;
		        case '2': common.wlog('home_gnb_espoir');	break;
		        case '3': common.wlog('home_gnb_new');      break;
		        case '4': common.wlog('home_gnb_best');     break;
		        case '5': common.wlog('home_gnb_planshop'); break;
		        case '6': common.wlog('home_gnb_event');    break;
		        case '7': common.wlog('gnb_foru');          break;
        	}
        }else{
        	switch (init_gnb_tab_idx) {
	            case '1': common.wlog('home_gnb_hotdeal');  break;
        ...
        mmain.main.mainGnbSwiper = new Swiper('#mainSwiper', {
            ...
            onInit : function(swiper){
                ...
                mmain.main.showLazyImages($(swiper.slides[init_gnb_tab_idx]));
            },
            onSlideChangeStart : function(swiper){
                $(document).scrollTop(0); // 2020-07-23 GNB Flick 최상단 포커싱
                //[3421036]올알랭 재생 오류 수정
                if (mmain.home.youtube_player && mmain.home.youtube_player.f) {
                    mmain.home.youtube_player.destroy();
                }
                mmain.main.showLazyImages($(swiper.slides[swiper.activeIndex]));
            },
            onSlideChangeEnd : function(swiper){
                ...
                let pos = gnbTabIdx;
                if(idx == 0){
                    pos = gnbTabIdx <= 0 ? mmain.main.gnbLastIdx : gnbTabIdx - 1;
                    mmain.main.makeSwipeHtml(pos,swiper);
                }else if(idx == 2){
                    pos = gnbTabIdx >= mmain.main.gnbLastIdx ? 0 : gnbTabIdx + 1;
                    mmain.main.makeSwipeHtml(pos,swiper);
                }
                //[3421036]올알랭 재생 오류 수정
                // if (mmain.home.youtube_player && $("#yt-player").length > 0) {
                //     mmain.home.onYouTubeIframeAPIReady();
                // }

                var gnb_Lw = $("#mGnb .gnb_cate").width();
                if (pos > 3) {
                    $("#mGnb .gnb_cate").scrollLeft(gnb_Lw);
                } else {
                    $("#mGnb .gnb_cate").scrollLeft(0);
                }
                $('#main-swiper-tab0 .mVisual-slide').ixOverlayList();
                
                // [3650436] 에스쁘아 GNB Swiper 적용 시점 조정
                (typeof mayflyLongbeards === 'object') && mayflyLongbeards.letThemFlap();
            },
    ...
    bindEvent : function() {
        ...
        var closeOneDayFirstPurchase = false;
        ...
        $("#oneday_close_FirstPurchase").click(function() {
        	closeOneDayFirstPurchase = $("input:checkbox[id='oneday_close_FirstPurchase']").is(":checked");
        });
        ...
        $("#healthvimeal_close").click(function() {
            closeOneWeek = $("input:checkbox[id='healthvimeal_close']").is(":checked");
        });
        ...
        //첫구매 유도 일반 닫기 클릭
        $("#btnFirstPurchaseClose").click(function() {
        	common.wlog("firstPurchase_ban_close");
        	if(closeOneDayFirstPurchase){
        		mmain.main.closeOneDayFirstPurchasePop('firstPurchase_sticky');
        	}else{
        		mmain.main.closeFirstPurchasePop('firstPurchase_sticky');
        	}
        });
        ...
        //건강비밀 넛지팝업 일주일간 보지않기 클릭
        $("#btnHealthSecretClose").click(function() {
            common.wlog("btnHealthSecretClose");

            if(closeOneWeek){
                mmain.main.closeHealthSecretPop('health_secret_popup_sticky');
            } else {
                mmain.main.closeHealthSecretDefaultPop('health_secret_popup_sticky')
            }
        });
...
//      }
    ...
    closeWeekPop : function(popupId){
        ...
        var shopAlimOlLiveList = sessionStorage.getItem("shopAlimOlLiveList");
        ...
        if($("#footerTab #appPush_sticky").length < 1){
        	...
        	if (common.isEmpty(shopAlimOlLiveList)) {
        		$("#noti_service").remove();
        	}
 ...
 // [3453582] 첫구매 유도 토스트팝업 일주일 안보기 
    closeOneDayFirstPurchasePop : function(popupId){
    	mmain.main.setCookieMobile("firstPurchaseOneDayCookie", "done" , 1);
        $(document).scrollTop(0);
        
        var mFirstPurchaseBan = $('#firstPurchase_sticky'),
		mFooterTab = $('#footerTab'),
		mfirstPurchaseHeight = mFirstPurchaseBan.height();
        $("#"+popupId).remove();
        
        var shopAlimOlLiveList = sessionStorage.getItem("shopAlimOlLiveList");

        if($("#footerTab #firstPurchase_sticky").length < 1){
        	$("#footerTab").removeClass("btmBan");
        	$('#fixBtn').removeClass('manban');
        	$('#footerTab').removeAttr("style");
    ...
    closeHealthSecretPop : function(popupId){
        mmain.main.setCookieMobile("healthSecretCookie", "done" , 7);
        $(document).scrollTop(0);
        $("#"+popupId).remove();

        var shopAlimOlLiveList = sessionStorage.getItem("shopAlimOlLiveList");

        if($("#footerTab #health_secret_popup_sticky").length < 1){
            $("#footerTab").removeClass("btmBan");
            $('#fixBtn').removeClass('manban');
            $('#footerTab').removeAttr("style");
    ...
    closeHealthSecretDefaultPop : function(popupId){
        $(document).scrollTop(0);
        $("#"+popupId).remove();
 ...
 // [3453582] 첫구매 유도 토스트팝업 일반 팝업닫기 
    closeFirstPurchasePop : function(popupId){
        $(document).scrollTop(0);
        var mFirstPurchaseBan = $('#firstPurchase_sticky'),
		mFooterTab = $('#footerTab'),
		mfirstPurchaseHeight = mFirstPurchaseBan.height();
    ...
    // 첫구매유도 팝업 DS
    setFirstPurchaseBanDs : function(){
    	common.wlog("firstPurchase_ban_img");
    },
    ...
    // 앱 푸시 유도 팝업 DS
    setAppPushBanDs : function(){
        common.wlog("app_push_ban_img");
    },

    // 건강비밀 팝업 DS
    setHealthSecretDs: function() {
        common.wlog("health_secret_img");
    },
    ...
    getCookieMobile : function(cookieId, popupId) {
        ...
        //첫구매 유도 배너
        var firstPurchaseBanInfo = sessionStorage.getItem("firstPurchaseBanInfo");
        //첫구매 유도 배너 확인 후 앱종료 했다는 flag
        var byThroughAppFirstPurchase = sessionStorage.getItem("byThroughAppFirstPurchase");
        ...
        var cookiedata3 = document.cookie.match('(^|;) ?' + "firstPurchaseCookie" + '=([^;]*)(;|$)') == null ? "" : document.cookie.match('(^|;) ?' + "firstPurchaseCookie" + '=([^;]*)(;|$)')[2];
        ...
        if ( "done" == cookiedata3 ){
        	//첫구매 유도 배너
        	firstPurchaseBanInfo = "done";
        }
        
        var mManban = $('#man_sticky'),
    		mAppban = $('#appPush_sticky'),
    		mFirstPurchaseban = $('#firstPurchase_sticky'),
    		
    		mFooterTab = $('#footerTab'),
    		mBanHeight = mManban.height(),
    		mAppBanHeight = mAppban.height();
        
        if(( $("#footerTab #firstPurchase_sticky").length > 0 && firstPurchaseBanInfo == "done"
        	&& byThroughAppFirstPurchase != "done") ){
        	$("#firstPurchase_sticky").remove();
        	$('#footerTab').removeClass('mfixAll');
            ...
            //sessionStorage.removeItem("manBanInfo", "undefined");
            sessionStorage.removeItem("manBanInfo");
            sessionStorage.removeItem("appBanInfo");
        ...
        if(( $("#footerTab #firstPurchase_sticky").length > 0 && firstPurchaseBanInfo != "done" ) ){
            ...
            sessionStorage.setItem("firstPurchaseBanInfo", "done");
        }else if( $("#footerTab #man_sticky").length > 0 && manBanInfo != "done" ){
        	sessionStorage.setItem("manBanInfo", "done");
        	
        }else if( $("#footerTab #appPush_sticky").length > 0 &&appBanInfo != "done" ){
        	sessionStorage.setItem("appBanInfo", "done");
        	
        }else if ( (manBanInfo != undefined && manBanInfo != "undefined" && manBanInfo == "done" && appBanInfo == "done") 
        	) {
        	$("#"+popupId).remove();
        	...
        	setTimeout(function() {
        		if(sessionStorage.getItem("firstPurchaseBanInfo") != "done"){
        		common.gnb.callFooterBann();
        		}
        	}, 600);
        }
        
        if(firstPurchaseBanInfo == null && byThroughAppFirstPurchase == "done"){
        	sessionStorage.removeItem("byThroughAppFirstPurchase");
        	common.setSessionCookie("firstPurchaseTempCookie", "done");
        }
    ...
    showLazyImages : function (wapperElement) {
        if(!wapperElement || wapperElement.length < 1 ) {
            return;
        }
        var lazyImageElementList = wapperElement.find('.lazy_loading');
        if(!lazyImageElementList || lazyImageElementList.length < 1) {
            return;
        }
        for(var i=0, len=lazyImageElementList.length; i<len; i++){
            $(lazyImageElementList[i]).show();
        }
    },
    // video play
...
mmain.menu = {
    ...
    bindWebLog : function() {
        ...
        $("#mGnb").find("li[data-ref-url='main/getHotdealAjax.do']").bind("click", function() {
            common.wlog("home_gnb_hotdeal");
            mmain.menu.removeTodaySpecialSessionValue();
        });
...
//        }catch(e){}
        ...
        $("#mGnb").find("li").removeClass("on").find('a').removeAttr('title');
        $("#mGnb").find("li").eq(gnbTabIdx).addClass("on").find('a').attr('title','선택됨');
        
      //[3411696] '20년상반기 웹앱접근성 개선필요항목(모바일웹) 타이틀 변경
        $('html > head > title').text($("#mGnb").find("li").eq(gnbTabIdx).find("a > span").text() + ' | 올리브영');
...
//                  alert(e);
                ...
                if(mmain.main.stonUseYn == 'Y' &&
                        (  
                            ...
                            /* 비회원 상시 웹캐시 적용되어 있던 신상, 기획전 페이지 2021.09.14일 오특개편 및 모바일 상품카드 변경 시점부터 캐시 적용 대상에서 제외 */
...
//                        $("#mContainer").append("<script>" + tmpScript + "</script>")
            ...
            if(gnbTabIdx != 6) { // 추천 탭일 때는 저장하지 않게 처리
            ...
            //실시간 랭킹은 무조건 새로 불러온다.
            /*
            if(gnbTabIdx == 3 && $("#mFixTab #mTab").find("li:eq(2)").hasClass("on")) {
            	$("#rankBestDiv").empty();
            	mmain.best.getRankBestInit();
            }
            */
            var date 	= new Date();
            var today	= "" + date.getFullYear() + ("0" + (1 + date.getMonth())).slice(-2) + ("0" + date.getDate()).slice(-2)    + ("0" + date.getHours()).slice(-2);
            if (today >= '2021100709' && today <= '2021100823') {
                if(gnbTabIdx == 4 && $("#mFixTab #mTab").find("li:eq(2)").hasClass("on")) {
                	$("#rankBestDiv").empty();
                	mmain.best.getRankBestInit();
                }
            }else{
...
//        }, 1000);
        ...
        if(gnbTabIdx < 6) {									// (1->6  변경) 홈 : 0, 오특 : 1, ~ , 이벤트 : 6
...
//                  alert(e);
                ...
                if(linkUrl == "mycuration/getCurationForYouAjax.do") {
                    return;
                }
                   $.ajax({
                       type: "GET",
...
//        }
        ...
        var prevValue = sessionStorage.getItem("gnb_tab_idx");	//megabrand
...
//                sessionStorage.clear();
            ...
            if (checkHashArr.length < 2) {
                ...
                //메가브랜드데이 기간동안 페이지 링크 변경
                var date	= new Date();
                var today	= "" + date.getFullYear() + ("0" + (1 + date.getMonth())).slice(-2) + ("0" + date.getDate()).slice(-2)    + ("0" + date.getHours()).slice(-2);
                if (today >= '2021100709' && today <= '2021100823') {
                	if (hash == 14) {	//9:바이오힐보, 10:웨이크메이크, 11:닥터자르트, 12:클리오, 13:라로슈포제, 14:에스쁘아
                        gnbTabIdx = 1;
                    } else if (hash > 0 && (prevValue == '' || prevValue == undefined || prevValue == null ) && hash < 40) {
                        gnbTabIdx = hash * 1  + 1;
                    }
                } else if (hash == 14) {	//9:바이오힐보, 10:웨이크메이크, 11:닥터자르트, 12:클리오, 13:라로슈포제, 14:에스쁘아
                    gnbTabIdx = 0;
                }
				...
				//[3581907] 올영체험단 히스토리백 오류 건
				if		(gnbTabIdx == '5'  &&  gnbSubCatNo == '2'  &&  prevValue == '5'  &&  sessionStorage.getItem("gnb_sub_cat_no") == null){
					gnbTabIdx = 0;

				}else if(gnbTabIdx == '6'  &&  gnbSubCatNo == '2'  &&  prevValue == '6'  &&  sessionStorage.getItem("gnb_sub_cat_no") == null){
	                var date	= new Date();
	                var today	= "" + date.getFullYear() + ("0" + (1 + date.getMonth())).slice(-2) + ("0" + date.getDate()).slice(-2)    + ("0" + date.getHours()).slice(-2);
					if(today >= '2021100709' && today <= '2021100823'){
						gnbTabIdx = 0;
            ...
            if (gnbTabIdx == 49) history.pushState('', '', _plainUrl + "main/main.do#0");
...
//            }, 100);
    ...
    goToDirectNum : function(hash) {
    	//console.log("goToDirectNum =================================================================================================================================: " + hash);
        var varGnbTabIdx;
        try{
        ...
        case 15:
            //초기로딩시
            sessionStorage.setItem("gnb_tab_idx", hash);console.log("DEBUG > main.js > hash - 0: " + (hash - 0) + " & sessionStorate gnb_tab_idx: " + sessionStorage.getItem("gnb_tab_idx") );
            break;
        case 41:
            //핫딜
        ...
        case 52:
            ...
            break;
        case 53:
            //메가브랜드	megabrand : config.get("gnbIdxMegaBrand")
            mmain.menu.setDirectGnbTabIdx("planshop/getPlanShopDetail.do");
            break;
        default :
            //사파리 private mode 예외처리용
            try {
...
//            sessionStorage.setItem("saved_slide_html", $('#mContainer').html());
            // 추천 탭일 경우 html 저장 안함
            sessionStorage.setItem("saved_slide_html", gnbTagIdx == 6 ? "" : mmain.main.getCurrentSlideHtml());
        } catch (e) {}
...
//        .css('transition-duration','500ms');
    ...
    removeTodaySpecialSessionValue() {
        var currentGnbTabIndex = location.hash.slice(location.hash.length-1);

        sessionStorage.removeItem("slide_html_log_" + currentGnbTabIndex);
        sessionStorage.removeItem("slide_html_" + currentGnbTabIndex);
        sessionStorage.removeItem("slide_script_" + currentGnbTabIndex);
        sessionStorage.removeItem("saved_status");
        sessionStorage.removeItem("saved_tab_idx");
        sessionStorage.removeItem("saved_slide_html");
    }
    ...
    init : function(gnbTabIdx) {
    	...
    	if(gnbTabIdx){
            ...
            */
            if(gnbTabIdx){    	
                if (today >= '2021100709' && today <= '2021100823') {    	
                    tabClass = "#main-swiper-tab" + (gnbTabIdx + 1) + " ";    	
                } else {    	
                    tabClass = "#main-swiper-tab" + gnbTabIdx + " ";    	
                }
            }
...
//        mmain.subGnb.mSubGnb_swiper =
		...
		*/
        if (today >= '2021100709' && today <= '2021100823') {	
            mmain.subGnb.scrollToMenu(initSlide, gnbTabIdx + 1);	
        } else {	
            mmain.subGnb.scrollToMenu(initSlide, gnbTabIdx);	
        }
        ...
        $(tabClass + "#mSubGnb").find("li").each(function(idx) {
            $(this).find("a").click(function() {
                ...
                */
                if (today >= '2021100709' && today <= '2021100823') {
                    mmain.subGnb.scrollToMenu(idx, gnbTabIdx + 1);
                } else {
                    mmain.subGnb.scrollToMenu(idx, gnbTabIdx);
                }
            ...
            if($(this).hasClass('on')) {
                categoryTxt = $(this).text();
                sessionStorage.setItem("bestCategoryTxt", categoryTxt);
            }
        });


        //GTM Tagging
        $('.mlist2v-best.prod-list.v2 .prod').each(function (i) {
            var _item = $(this);
            var _data_goodsno = _item.find('a').attr('data-ref-goodsno');
            var goodNm =  _item.find('.prod-info a .prod-name.double-line').text();
            _item.find('a').attr("data-attr" , '랭킹^판매랭킹리스트_' + categoryTxt + '^' +goodNm);
            _item.find('a').attr('onclick','gtm.goods.callGoodsGtmInfo("'+_data_goodsno+'", "", "ee-productClick", "랭킹_판매랭킹리스트_'+categoryTxt+'","'+(i+1)+'");');
            _item.find('a.thumb').attr("data-impression" , _data_goodsno+'^랭킹_판매랭킹리스트_' + categoryTxt + '^' +(i+1));
        });
        ...
        if (location.hash == currentHash) {
            $('.goods-list-container li a.thumb').each(function (i) {
                var _item = $(this);
                var categoryTxt = sessionStorage.getItem("bestCategoryTxt");

                var _data_goodsno = _item.attr('data-ref-goodsno');
                var _data_attr = '랭킹^판매랭킹리스트_' + categoryTxt + '^' + _item.siblings().find('p').clone().children().remove().end().text()
                var _data_category_action = '랭킹_판매랭킹리스트_' + categoryTxt
                var _data_impression = _data_goodsno + '^랭킹_판매랭킹리스트_' + categoryTxt + '^' + (i + 1);

                _item.attr('data-attr',_data_attr);
                _item.attr('data-impression',_data_impression);
                _item.attr('data-category-action',_data_category_action);
                _item.find('img').attr('data-attr',_data_attr);

                _item.siblings('.info').attr('data-attr',_data_attr);
                _item.siblings('.info').find('p').attr('data-attr',_data_attr);
                _item.siblings('.info').find('.flag').attr('data-attr',_data_attr);
            });

        }

        gtm.goods.initProductImpression();
        newGoods.common.init();
...
mmain.home = {
    ...
    init : function() {
        ...
        if(mmain.home.recoBellUseYn == 'Y' && mmain.home.recoBellViewYn == 'Y'){
            ...
            mmain.home.getStoreRecommInfoAjax(); // 매장추천영역 : 큐레이션 + 새로 오픈한 매장 함께 조회
            
            // 최근 클릭 카테고리 기반
            mmain.home.getRecoBellRecentCategoryAjax();
        } else {
        	mmain.home.completeStoreRecommAjaxCall(); // 매장추천영역 : 새로 오픈한 매장만 조회한다.
        }
        
        if(mmain.home.rankRecoBellViewYn == "Y") {
        	mmain.home.getHomeTimeRankInfo();
        }
        
        setTimeout(function() {
            var randomIdx = Date.now() % $(tabClass+".best-slide").length;
            randomIdx = 0; // [3428914] 스킨케어로 고정
...
//            $(".goodsList").removeClass("goodsList");
		...
		$("body").on("click touch", "button.hide-all-banner", function(){
			...
			//올알랭2차 Android 스크롤 오류 20201104 S
			setTimeout(function() {
                common.app.callMenu("N");
            }, 100);
			//올알랭2차 Android 스크롤 오류 20201104 E
		});
		// [3393110] End
		
		$("div.brndList").hide(); // [3407293] 브랜드 리스트 초기값 숨김처리
		
		// [3419941] 오특 카운트다운 타이머
	    ...
	    let x = setInterval(function() {
	        ...
	        function mm(elem, time){
	            let t;
	            if (time < 10) t = "0" + time;
	            else t = String(time);
	            let t1 = t.slice(0, 1);
	            let t2 = t.slice(1, 2);
	            elem.find(".num").eq(0).attr("data-timer-val", t1).text(t1);
	            elem.find(".num").eq(1).attr("data-timer-val", t2).text(t2);
	        };
	        let timer = $(".main-today .timer");
	        mm(timer.find(".nums.h"), hours);
	        mm(timer.find(".nums.m"), minutes);
	        mm(timer.find(".nums.s"), seconds);
	    }, 1000);
    },
            ...
            mmain.home.youtube_player = new YT.Player('yt-player', {
                ...
                events: {
                    'onReady': function(event){
                        //console.log('onReady');
                        mmain.home.bindYoutubeEvent(event);
	...
	onPlayerReady : function (event) {
        ...
        var mainIdx = $(".mtab").eq(1).attr('data-menu-index');     // main 인덱스 가져옴

		if (some1 > 0 && some2 > 0 && $(window).scrollTop() <= some1 && $(window).scrollTop() >= some2) {
			//로드 될 때 화면에 보이니?
			...
			if(location.hash === '#0'){
                ...
                window.dataLayer.push(
                    {
                        'event' : 'click-event',
                        'data-attr' : '홈^유튜브관^' + mmain.home.youtube_player.getVideoData().title
                    }
                );
            }
		} else {
...
//          mmain.home.youtube_player.stopVideo();
    ...
    bindYoutubeEvent : function(){
		...
		let newGoodTimeStamp = $(tabClass + ".allalain .video");
        ...
        newGoodTimeStamp.on("click touch", function(){
            event.stopPropagation(); // 이벤트 버블링 막기
            let seekto = $(this).attr("data-timeline");
            if ( seekto ) {
                mmain.home.youtube_player.seekTo(seekto, true);
        ...
        var mainIdx = $(".mtab").eq(1).attr('data-menu-index');
        var youtubePlaytime = 0;
        var youtubePlay = false;
        var youtubePlaytimer = setInterval(function(){
            if(youtubePlay && location.hash === '#0'){
                youtubePlaytime += 1;
                if(youtubePlaytime >= 10){
                    window.dataLayer = window.dataLayer || [];
                    window.dataLayer.push(
                        {
                            'event' : 'click-event',
                            'data-attr' : '홈^유튜브관^' + mmain.home.youtube_player.getVideoData().title + "_10초"
                        }
                    );
                    clearInterval(youtubePlaytimer);
                    youtubePlay = false;
                }
            }
        }, 1000);
		...
		$(window).on("scroll", function(){
		    try {
    		    if (location.hash == "#0") { // 메인 홈에서만
    	            var player;
    	            if (event.target) {
    	                player = event.target;
    	            } else {
    	                player = mmain.home.youtube_player;
    	            }
    	            
    	            var top = $("#yt-player").offset().top; // 플레이어를 맨 상단으로
                    var bottom = $("#yt-player").offset().top - $(window).innerHeight() + $("#yt-player").height() ;   // 플레이어를 맨 상단으로하고 윈도우크기만큼 내리고 플레이어 화면만큼 올리고
                    var scrollTop = $(window).scrollTop();
                    var overH = 20; // 경계에서 이정도 겹쳐도 재생

                    //console.log("$('#yt-player').top : " + top);
                    //console.log("$('#yt-player').bottom : " + bottom);
                    //console.log("$(window).scrollTop() : " + scrollTop);
                    
                    //if ($('#mHeader').hasClass("scroll_down")) {
                    if ($('#footerTab').hasClass("on")) { // 스크롤업일 때 하단 유틸바가 나타남
                        var bH = $("#webBanner_main").height();
                        if ($("#webBanner_main").css("display") == "none") {
                            bH = 0;
                        }
                        
                        if ((top - 27 - 60 - bH + overH) >= scrollTop && bottom + 50 - overH <= scrollTop) {
                            //console.log("플레이2");
                            mmain.home.youtube_player.playVideo();
                            youtubePlay = true;
                        } else {
                            //console.log("중지2");
                            mmain.home.youtube_player.pauseVideo();
                            youtubePlay = false;
                        }
                    } else {
                        if (scrollTop >= bottom - overH && scrollTop <= top - 37 + overH) { // 
                            //console.log("플레이1");
                            mmain.home.youtube_player.playVideo();
                            youtubePlay = true;
                        } else {
    ...
    //올알랭2차 CBLIM S
	getAllalainAjax : function(){
		
        var url = _baseUrl + "main/getAllalainAjax.do";
        var data = { allAlainCheck : $(".allalain .home-stit-18").html()};
        common.Ajax.sendRequest("POST",url,data,mmain.home._callBackGetAllalainInfo);
        
		
	},
    oytabIdx: 0,
    _callBackGetAllalainInfo : function(data) {
        $("#layer-allalain").empty();
        $("#layer-allalain").append(data);
        var sItem = $('.itemSlide').find('.swiper-slide button');
        var allAlainCheck = $("#allAlainCheck").val();
        var maxTabSize = $("#maxTabSize").val();

        for (var i = 0 ; i < maxTabSize ; i ++) {
            if (allAlainCheck == $("#tabContList_" + (i + 1)).val()) {
                mmain.home.oytabIdx = i;
            }
        }
        var itemSlide = new Swiper('.itemSlide', {
            slidesPerView : 'auto',
            initialSlide : mmain.home.oytabIdx,
        ...
        common.oyFullOpen('layer-allalain');

        sItem.on('click', function(){
            mmain.home.oytabIdx = $(this).parent('.swiper-slide').index();
        });
        setTimeout(function() {
            mmain.home.youtube_player.pauseVideo();
        }, 100);
...
//        var isLogin = common.isLogin(); //중복호출 제거
	     if(mmain.home.recoCallYn == "N"){
			...
			curation.callCuration(recType, param, function(result) {
	    		if(result != undefined && result.results != undefined && result.results != null && result.results.length > 0) {
	    			var url = _baseUrl + "main/getRecoBellContsInfoAjax.do";
                    var obj = {
                        callBackFunc :  mmain.home._callBackGetRecoBellContsInfo,
                    };
    ...
    getEigeneTimeBestInfoAjax : function() {
		...
		curation.callCuration(recType, param, function(result) {
    		if(result != undefined && result.results != undefined && result.results != null && result.results.length > 0) {
                ...
                var obj = {
                    callBackFunc :  mmain.home._callBackGetEigeneTimeBestInfo,
                };
    ...
    _callBackGetEigeneTimeBestInfo : function(obj, data) {
    	$("#recobell_area1").html("");
        $("#recobell_area1").html(data);
        //찜 처리 초기화
        common.wish.init();
    ...
    getRecoBellClickBestInfoAjax : function() {
		...
		curation.callCuration(recType, param, function(result) {
    		if(result != undefined && result.results != undefined && result.results != null && result.results.length > 0) {
                ...
                var obj = {
                    callBackFunc :  mmain.home._callBackgetRecoBellClickBestInfo,
                };
    ...
    getRecoBellRecentInfoAjax : function() {
		...
		curation.callCuration(recType, param, function(result) {
    		if(result != undefined && result.results != undefined && result.results != null && result.results.length > 0) {
                ...
                var obj = {
                    callBackFunc :  mmain.home._callBackgetRecoBellRecentInfo,
                };
    ...
    getRecoBellRecentCategoryAjax : function() {
        // 레코벨 스크립트에서 호출하도록 변경
        // 파라미터 생성
        var recType = "p071";
        var param = {
            recType : recType,
          //[3610789] (큐레이션) 메인페이지 카테고리기반 추천영역 결함
          //size : 30,
            size : 50,
            cps : true,
            cpt : "t001",
        ...
        curation.callCuration(recType, param, function(result) {
            if(result != undefined && result.results != undefined && result.results != null && result.results.length > 0) {
                var url = _baseUrl + "main/getRecoBellRecentCategoryAjax.do";
                var obj = {
                    callBackFunc :  mmain.home._getRecoBellRecentCategoryAjax,
                };
    ...
    _getRecoBellRecentCategoryAjax : function(obj, data) {          
        $("#recobell_area3").html("");
        $("#recobell_area3").html(data);

        if(obj.category2 && recType !== "t001") {
            var url = _baseUrl + "api/getCategoryName.do?dispCatNo=" + obj.category2;
            common.Ajax.sendRequest("GET", url, null, function(data) {
                if(data.body.message === "성공"){
                    $("#recobellCategoryTitle").text(data.body.categoryName);
                }else{
                    $("#recobellCategoryTitle").text("카테고리"); 
                }
            });
        }
    ...
    getStoreRecommInfoAjax : function() {
    	...
    	curation.callCuration("p039", {}, function(data) {
    		if(data != undefined && data.results != undefined && data.results != null && data.results.length > 0) {
    			...
    			mmain.home.completeStoreRecommAjaxCall(strNo);
    		} else {
    			mmain.home.completeStoreRecommAjaxCall();
    		}
    	}, mmain.home.completeStoreRecommAjaxCall);
    },
    completeStoreRecommAjaxCall : function(strNo) {
    	if(common.isEmpty(strNo)) {
    		strNo = "";
    	}
    	
    	var url = _baseUrl + "main/getStoreRecommInfoAjax.do";
    	var data = {strNo : strNo};
    	common.Ajax.sendRequest("POST", url, data, mmain.home._callBackGetStoreRecommInfo);
    },
    _callBackGetStoreRecommInfo : function(res) {
    	$(".newOpenStore").html("");
    	$(".newOpenStore").html(res).show();
    ...
    getHomeTimeRankInfo : function() {
    	...
    	curation.rts.callRtsCuration("stats", requestParam, function(data) {
    		var result = data.items;
	       	result.type = "view";
	    	curation.rts.getCurationJsonCallBack(result, mmain.home.setHomeTimeRankInfo);
    	});
    },
    rankRefresh : false,
    scrollRefresh : false,
    setHomeTimeRankInfo : function(res) {
        ...
        var rccode = common.isLogin() ? "mc_main_04_a" : "mc_main_04_c";
        var liveRankTrackingCd = 'Home_Live_Rank';
        var liveGtmCategoryAction = '홈_실시간VIEW랭킹';

        var prod = {
    		rankList : prodList,
            viewArea : "home_real_time_rank",
            rccode: rccode,
            trackingCd : liveRankTrackingCd,
            gtmCategoryAction : liveGtmCategoryAction,
            dataWebLog : 'homeViewRankingGoods'
        };
    	
    	if(len > 0) {
            $("#liveRankArea").html($('#rankViewTmpl').tmpl(prod));

    		$("#liveRankArea").show();
    		
    		setTimeout(function() {
    			$(".rank-slide").eq(0).addClass("active");
    		}, 100);
    		
    		$("#btn_goBestTab").unbind("click").click(function(event) {
    			event.preventDefault();
    			
    			mmain.best.rankInit = true;
    			/* megabrand
                ...
                */
                ...
                var today 	= "" + date.getFullYear() + ("0" + (1 + date.getMonth())).slice(-2) + ("0" + date.getDate()).slice(-2)    + ("0" + date.getHours()).slice(-2);
	            if (today >= '2021100709' && today <= '2021100823') {	
                    document.location.href= _baseUrl + "main/main.do#4";	
                } else {	
                    document.location.href= _baseUrl + "main/main.do#3";	
                }
    		...
    		var $anchors = $('.liveRankBox .rank-slide');
    		var tabTimeout;
    		var idx;
    		var chkIndex;
    		...
    		$(window).scroll(function() {
    			if($("#liveRankArea").is(":visible")) {
    				var wH = $(window).height(),
    				wS = $(this).scrollTop();
    				
    				var offsetTop = $('#liveRankArea').offset().top;
    				
    				if(wS >= ( offsetTop-wH )) {
    					mmain.home.rankRefresh = true;
    					
    					if(mmain.home.scrollRefresh) {
    						clearTimeout(tabTimeout);
    						tabLoop(0);
    					}
    				} else {
    					mmain.home.rankRefresh = false;
    				}
    			}
        	});

    	    // define loop - cycle through all tabs
    	    function tabLoop(idx) {
    	        tabTimeout = setTimeout(function() {
    	            if(idx != $anchors.length) {
    	                $anchors.eq(idx).find('.slideRanker').click();
    		        	$anchors.removeClass('active').eq(idx).addClass('active');
    	                idx = idx + 1;
    	                mmain.home.scrollRefresh = false;
    	            } else {
    	            	if($("#liveRankArea").is(":visible") && mmain.home.rankRefresh) {
	    	            	mmain.home.getHomeTimeRankInfo();
	    	            } else if(!mmain.home.rankRefresh) {
	    	            	mmain.home.scrollRefresh = true;
	    	            }
    	            }
    	        }, 4000);
    	    }
    	    $('.slideRanker').click(function() {
    	        clearTimeout(tabTimeout);
    	        chkIndex = $(this).parents('li.rank-slide').index()+1;
    	        $anchors.removeClass('active').eq(chkIndex-1).addClass('active');
    	        tabLoop(chkIndex);
            });
    	    

    	    curation.prodDetailScript({viewArea : "home_real_time_rank", rccode: rccode, parentArea : "#liveRanking", trackingCd : "Home_Live_Rank"});
    	    gtm.goods.initProductImpression();
      	...
      	if(html && html.trim() != ""){
          	...
          	$(setObj.viewArea).find(".prod-inner").each(function(idx) {
                ...
                $(this).find("a.goodsList").each(function(i){
    	            ...
    	            _item.attr('href','javascript:common.wlog("'+web_log_id+'");common.link.moveGoodsDetailCuration("'+_data_goodsno+'", "'+_data_dispCatNo+'","'+web_log_id+'","'+rccode+'", "'+egcode+'", "'+egrank+'","","display","","goods_image", "'+trackingCd+'");');
    ...
    bindEvent : function() {
        ...
        $(".best-slide").click(function() {
            mmain.home.bestSlideClick($(this));
        });
...
//        });
        ...
        // [3421036] 카테고리 랭킹이 한번 온전히 노출되면 스와이핑 동작
        $(window).on("scroll", function(){
            try {
                if (location.hash == "#0") { // 메인 홈에서만
                    var top = $("div.main-cate-rank").offset().top; // 카테고리 랭킹 맨 상단으로
                    var bottom = $("div.main-cate-rank").offset().top - $(window).innerHeight() + $("div.main-cate-rank").height() ; // 카테고리 랭킹을 맨 상단으로하고 윈도우크기만큼 내리고 화면만큼 올리고
                    var scrollTop = $(window).scrollTop();
                    
                    if ($('#mHeader').hasClass("scroll_down")) {
                        if (scrollTop >= bottom && scrollTop <= top - 37) {
                            mmain.home.isCateRankPaused = false;
                        }
                    }
                }
            } catch (e) {
    ...
    // 카테고리 랭킹 클릭 // 자동 클릭시 DS 오류 수정
    bestSlideClick : function(obj) {
        $(".best-slide").removeClass(mmain.home.getMSub1sMenuActiveClass());
        $(obj).addClass(mmain.home.getMSub1sMenuActiveClass());
        $(mmain.home.getMSub1sListClass()).empty();
        mmain.home.getBestTop3List($(obj).attr("data-ref-dispCatNo"));
        mmain.home.scrollToTabMenu($(obj).attr("data-ref-idx"), mmain.home.getMSub1sMenuId(), ".sub_gnb_cate.swiper-wrapper.best");
    },
...
//    	$('#main-swiper-tab0 .ix-controller .ix-thumb').not(':eq(0)').remove(); // [3393110]
		...
		mmain.home.mVisual = $('#main-swiper-tab0 .mVisual-slide'),
			ixItems = mmain.home.mVisual.find('.ix-list-items'),
			ixItem = mmain.home.mVisual.find('.ix-list-item'),
			ixItemLen = ixItem.length-1,
			pagination = mmain.home.mVisual.find('.ix-controller .idx');			//2020-08-26 추가 - [3393110]
		
		ixItem.eq(0).addClass('st');
		mmain.home.mVisual.ixOverlayList();
		mmain.home.mVisual.find('li[data-idx='+ixItemLen+']').addClass('prev');
	    pagination.text("1");											//2020-08-26 추가 - [3393110]
	    mmain.home.mVisual.find('.ix-controller .length').text(ixItem.length);		//2020-08-26 추가 - [3393110]
	    mmain.home.mVisual.on('ixOverlayList:slideStart', function(e){
	    ...
	    mmain.home.mVisual.on('ixOverlayList:slideEnd', function(e){
	        var index = e.currentIndex,
	        	direction = e.direction;
	        if(direction == 'next'){
	            var index = index<=0 ? index = ixItemLen : index = index - 1;
...
//        });
      ...
      //Quick-Menu Swiper
        ...
        // [3419941] 오특 swiper shuffle
        var hdLength = $("#today-curation .prod-list .prod").length;
        for (var i = 0; i < hdLength; i++) {
            var randomA = Date.now() % hdLength;
            while (i == randomA) {
                randomA = Date.now() % hdLength;
            }
            var a = $("#today-curation .prod-list .prod").eq(i);
            var b = $("#today-curation .prod-list .prod").eq(randomA);
            a.after(b);
        }

        var hotDealSwiper = new Swiper('#today-curation', {
            slidesPerView: 'auto',
            spaceBetween: 15,
            loop: false,
            scrollbar: '.swiper-scrollbar',
        ...
        //메인 오늘의특가 물품 갯수 없으면 표출되지 않게 조건 추가 
        if(hdLength > 0){
        	hotDealSwiper.slideTo(1); // 0만 하면 안움직임
            hotDealSwiper.slideTo(0);
        }

        $('#today-curation .prod-list .prod').each(function(i){
            var _data_goodsno = $(this).find('.prod-inner .thumb-wrap .thumb.goodsList').attr('data-ref-goodsno');
            var _data_goodsNm = $(this).find('.prod-info-wrap .prod-info a .prod-name').text();
            $(this).find('.prod-inner .thumb-wrap .thumb.goodsList').attr('data-attr','홈^오늘의특가^' + _data_goodsNm + '^' + (i + 1));
            $(this).find('.prod-info-wrap .prod-info .goodsList').attr('data-attr','홈^오늘의특가^' + _data_goodsNm + '^' + (i + 1));

            $(this).find('.prod-inner .thumb-wrap .thumb.goodsList').attr('onclick','javascript:gtm.goods.callGoodsGtmInfo("'+_data_goodsno+'", "", "ee-productClick", "홈_오늘의특가", "' + (i + 1) + '");');
            $(this).find('.prod-info-wrap .prod-info .goodsList').attr('onclick','javascript:gtm.goods.callGoodsGtmInfo("'+_data_goodsno+'", "", "ee-productClick", "홈_오늘의특가", "' + (i + 1) + '");');

            $(this).find('.prod-inner .thumb-wrap .thumb.goodsList').attr('data-impression', _data_goodsno + '^홈_오늘의특가_' + _data_goodsNm + '^' + (i + 1));
        });
    },
...
/////////// TEST 코드 todo 정리 필요
    ...
    getMSub1sListClass : function() {
   	 ...
   	 try{
   		 if($("#mSub1s-menu")[0]){
   			 menuId = ".prod-list.h1.bdb.bestList";
   	     }
 ...
 ///////////
    ...
    isCateRankPaused : true, // 카테고리 랭킹 멈춤용 한번만 사용
    ...
    getBestAjaxCallback : function(res) {
        ...
        if(mmain.home.getMSub1sIsRenewal() == "A"){
            ...
            var categoryNm = $('#mSub1s-menu-new .sub_gnb_cate').find('.active').find('a').text();

            $('.mlist5v-goods-new .prod-wrap .prod .thumb').each(function (i) {
                var _item = $(this);

                var _data_goodsno = _item.attr('data-ref-goodsno');
                var _data_attr = '홈^카테고리랭킹_' + categoryNm + '^' + _item.siblings().find('p').clone().children().remove().end().text();
                var _data_category_action = '홈_카테고리랭킹_' + categoryNm
                var _data_impression = _data_goodsno + '^홈_카테고리랭킹_' + categoryNm + '^' + (i + 1);
			...
			const rank = ranks.find(".goods-list");
...
//			let rankInterval;
			...
			function startInterval(){
				...
				mmain.home.rankInterval = setInterval(function(){
				    if (!mmain.home.isCateRankPaused) {
				        autoNextRank();
				    }
				}, 3000);
			};
				...
				try {
					if(nextNum == 0 && actNum >= 0){
                        if($(".best-slide")[Number($(".best-slide.active").attr("data-ref-idx"))+1]){
                            mmain.home.bestSlideClick($(".best-slide")[Number($(".best-slide.active").attr("data-ref-idx"))+1]);
                        }else{
                            mmain.home.bestSlideClick($(".best-slide")[0]);
                        }
					}else{
			...
			ranks.find(".goods-list:first-child").addClass("active");
			startInterval();
	        // 랭킹 텍스트 링크 클릭시
        ...
        if (isBest) {
        	...
        	try{
                     ...
                     $(".mlist5v-goods-new.bestList > li > .prod-wrap").each(function(goodsIdx) {
                        $(this).find(".goodsList").each(function() {
                        	$(this).bind("click", function() {
                                common.wlog('homeCatRankingGoods');
        		...
        		if($("#mSub1s-menu")[0]){
                    ...
                    //베스트 상품
                    $(".prod-list.h1.bdb.bestList > li > a").each(function(goodsIdx) {
                       $(this).bind("click", function() {
            ...
            */
            ...
            // HEALTHY LIFE [300005] 스페셜 테마샵 영역 전시 개선 건
            $(".hLifeSlide a").each(function(idx) {
                $(this).bind("click", function() {
                    common.wlog("home_healthy_banner" + (idx + 1));
                });
            });
            ...
            $(".oy-recomm-prod .prod-inner").each(function(idx) {
                $(this).find("a.goodsList").bind("click", function() {
                    common.wlog('homeRecommendGoods');
                });
            });
            ...
            try{
            	...
            	// isRenewal = "B";
          		 if($("#mSub1s-menu")[0]){
                     ...
                     //더보기
                     $(".prod-list.h1.bdb.bestList .btnLinkMore2").bind("click", function() {
            ...
            $("#mSub2s-menu-new ul li").bind("click", function() {
                common.wlog('homeBrandGoods_brname');
            });
            //인기브랜드 상품
            $(".prod-wrap.brndList").each(function(idx) {
                ...
                $(this).find(".prod-inner").each(function(goodsIdx) {
                   $(this).bind("click", function() {
                       common.wlog('homeBrandGoods');
                   });
                });
                //브랜드 더보기
                $(this).find(".banner").find('a').bind("click", function() {
                    common.wlog('homeBrandGoods_banner');
                });
            });
            ///////
            ...
            $(".allalain .prd-cont").each(function(index) {
                // 메인상품 매핑
            	$(this).find(".main-item .allalainGoodsList").bind("click", function() {
                    common.wlog("homeVideoGoods");
                });
                //조합상품 매핑	
                $(this).find(".addon .prod").each(function(sub_index){
            ...
            //[3428979] 메인 올알랭 더보기 
            $("#allalain_more").bind("click", function() {
                common.wlog('homeVideoGoods_more');
            });
            
            // [3419941] 메인 오특
            $("#aHotDealMore").bind("click", function() {
                common.wlog("home_hotdeal_more"); 
            });
            $("#today-curation .prod-list .prod .prod-inner").bind("click", function() {
                var i = parseInt($(this).parent().index()) + 1;
                common.wlog('homeTodaySpecialGoods');
            });
        }
    },
...
mmain.planshop = {
    ...
    init : function() {
        ...
        */
        ...
        var today 		= "" + date.getFullYear() + ("0" + (1 + date.getMonth())).slice(-2) + ("0" + date.getDate()).slice(-2)    + ("0" + date.getHours()).slice(-2);
        if (today >= '2021100709' && today <= '2021100823') {
            tabClass = "#main-swiper-tab5" + " ";
        } else {
            tabClass = "#main-swiper-tab4" + " ";
        }
        ...
        PagingCaller.init({
            callback : function(){
				...
				*/
                ...
                mmain.planshop.fltDispCatNo = $(tabClass+"#mSubGnb ul li.on a").attr("data-ref-dispCatNo");
                ...
                var param = {
                    ...
                    fltCondition : "01",
                    fltDispCatNo : (mmain.planshop.fltDispCatNo == undefined || mmain.planshop.fltDispCatNo == null) ? "" : mmain.planshop.fltDispCatNo
                };

                //미리보기 파라미터 추가
                mmain.menu.setPreviewParam(param);

                /* 기획전 페이징 2021.09.14일 모바일 상품카드 A/B test때 캐시적용 안함 */
    ...
    getPlanShopPagingAjaxCallback : function(res) {
        ...
        */
        ...
        if (res.trim() == '' || mmain.planshop.nextPageIdx > 7) {
            ...
            if (!common.isEmpty(mmain.planshop.fltDispCatNo)) {
                $("#recommPlanTit, #recommPlan, #planTit").hide();
            } else {
                $("#recommPlanTit, #recommPlan, #planTit").show();
            }
            
            //응답 결과가 있는 경우 append 처리 후 lazyload 처리
            //$(tabClass+".mlist2v-goods").append(res);
            $(tabClass+"#ulPlanList").append(res);
...
//        sessionStorage.setItem("saved_slide_html", $('#mContainer').html());
    ...
    goDetailPage : function(){
        $(".plan_ty02, .recomm_plan_list").find("li").each(function() {
            var planShopTrackingCd = 'Planshop_PROD';
            $(this).find("a").bind("click", function() {
                common.link.movePlanShop($(this).attr("data-ref-dispCatNo"), planShopTrackingCd);
            });
        });
    },
    ...
    bindWebLog : function() {
        ...
        */
        ...
        $(tabClass+".main_contents .plan_ty02 > li").each(function(planshopIdx){
            //기획전 배너 로그ID생성
            ...
            //리뉴얼 상품 카드 적용시 삭제 예정
            //기획전 상품
            $(this).find(".planshopwrap a").each(function(){
                //기획전 상품 바인드
                $(this).bind('click', function() {
                    common.wlog('planshopGoods');
                });
            });
        });
        
        // [3428974] 추천기획전
        $(tabClass+".main_contents .recomm_plan .recomm_plan_list li a").bind("click", function() {
            common.wlog("planshop_banner_recom_" + $(this).attr("data-wlog-id"));
        });
    }
};
...
mmain.sale = {
    ...
    init : function() {
        ...
        //[3411696] '20년상반기 웹앱접근성 개선필요항목(모바일웹)
        $('#mTab > li.on').find('a').attr('title','선택됨');
    ...
    bindEvent : function() {
        ...
        $(".sub_gnb_cate li a").bind("click", function() {
            ...
            //페이징 콜러 제거
            PagingCaller.destroy();
			//메인개편 2차 CBLIM 20201029 S
            $(".prod-list").empty();
            $(".goods-list-container").empty();
			//메인개편 2차 CBLIM 20201029 E
...
//            $(".goodsList").removeClass("goodsList");
    ...
    getSaleSubCategoryBarAjaxCallback : function(res) {
        ...
        var tags = $("#mSubGnb ul li a");
        var flag = $("#mTab .on").text();

        for(var i = 0; i < tags.length; i++){
            var tag = tags[i];
            $(tag).attr("data-attr", "세일^세일카테고리_" + flag + "^" + $(tag).text());
        }
    },
    ...
    getSalePagingAjaxCallback : function(res) {
        ...
        if (res.trim() == '') {
            ...
            //응답 결과가 있는 경우 append 처리 후 lazyload 처리
			//메인개편 2차 CBLIM 20201029 S
            $("#saleGoodsList").append(res);
			//메인개편 2차 CBLIM 20201029 E
            //BI Renewal. 20190918. nobbyjin. lazyLoad 분리
            //mmain.main.setLazyLoad();
            $('.gtm_sale_list .prod').each(function(i){

                var _item = $(this).find(' a');
                var _data_goodsno = _item.attr('data-ref-goodsno');
                var data_ref_itemno = _item.attr('data-ref-itemno');
                var _nm = $(this).find('.prod-name').text();
                var _sortTxt = $('#mSubGnb .on').text() || "전체";

                _item.attr('data-attr','세일^세일상품리스트_' + _sortTxt + '^' + _nm +'^' + (i+1));
                _item.attr('onclick', 'javascript: gtm.goods.callGoodsGtmInfo("'+_data_goodsno+'", "", "ee-productClick", "세일_세일상품리스트_'+_sortTxt+'", "'+(i+1)+'");');
                if(_item.hasClass("thumb")) {
                    _item.attr('data-impression', _data_goodsno + '^세일_세일상품리스트_' + _sortTxt + '^' + (i + 1));
                }
            });
            ...
            $('.goods-list-container li a.thumb').each(function(i){
                ...
                var _data_attr = '세일^세일상품리스트_' + _sortTxt + '^' + _item.siblings().find('p').clone().children().remove().end().text() +'^' + (i+1);
                var _data_category_action = '세일_세일상품리스트_' + _sortTxt;
...
mmain.newquick = {
    ...
    init : function() {
    	...
    	$('html > head > title').text($("#mHeader").find("h2").text() + ' | 올리브영');
    ...
    setSwipe : function() {
      //오늘드림 전문관
        ...
        //오늘 드림 일시 정지 사용할 경우 - [3457453] 오늘드림 전문관 앱웹접근성 개선 SR (JHM)
        $('#swiper-autoplay').on({
            'click' : function(){
                if(!$(this).hasClass('pause')){
                    $(this).addClass('pause');
                    only_swiper.startAutoplay();
                }else{
                    $(this).removeClass('pause');
                    only_swiper.stopAutoplay();
                }
            } 
        });
    },
    ...
    bindEvent : function() {
        ...
        //링크 처리
        $("img[data-ref-link-url]").bind("click", function() {
            //[3507164] 오늘드림 데이터스토리 클릭 지표 추가 (오늘드림 메인배너1 ~ 5 : quick_main_banner1 ~ 5)
        	var ds_idx = $(this).attr('ds-idx');
        	if( ds_idx <= 5 ){
        		//console.log("DEBUG ==> bindEvent : " + "quick_main_banner" + $(this).attr('ds-idx'));
        		//console.log("DEBUG ==> bindEvent : " + "quick_main_banner" + ds_idx);
        		common.wlog("quick_main_banner" + ds_idx);
        	}
        ...
        $(".theme_list li").bind('click',function(){
            ...
            if(moreFlag == "more"){
                ...
                if(themeType == "all"){
                    mmain.newquick.goQuickListMove(dispContCd, themeNm, "ALL", "O2O_Theme" + themeIdx + "_PROD");
                } else {
                    mmain.newquick.goQuickListMove(dispContCd, themeNm, "ONE", "O2O_Theme" + themeIdx + "_PROD");
                }
            }
...
//            $(".goodsList").removeClass("goodsList");
    ...
    , goQuickListMove : function(themeNo, themeNm, themeType, trackingCd){

        common.link.moveQuickListUrl(themeNo, themeNm, themeType, trackingCd);
        ...
        init : function() {
            ...
            */
            ...
            if (today >= '2021100709' && today <= '2021100823') {
                ...
                tabClass = "#main-swiper-tab3" + " ";
            }

            var count = 0;
            //판매베스트 상품 1~100
			//메인개편 2차 CBLIM 20201029 S
            $(document).on("click","#sellBestDiv .prod a",function(e){
        		var index="";
        		if($(this).parent().attr('class')=="prod-info"){
        			index = $(this).parent().parent().index() + 1; 
        		}else{
        			index = $(this).parent().index() + 1;
        		}
        		
        		common.wlog('sellingBestGoods');
            });
			//메인개편 2차 CBLIM 20201029 E
            //리뷰베스트 상품 1~100
            ...
            if(mmain.best.rankInit) {
            	$(tabClass+"#mTab li").removeClass("on");
            	$(tabClass+"#mTab li:eq(2)").addClass("on");
            	
            	mmain.best.getRankBestInit();
            	
            	mmain.best.rankInit = false;
            }
...
//                mmain.best.reviewBestSwipe();
                ...
                mmain.best.reviewSubGnbInit();
            }else if(dispCatNo == "900000100100001"){
                // 판매 베스트
...
//                        $(tabClass+".sub_gnb_cate_select option").eq(0).prop("selected","selected").trigger("change");
                    	$(tabClass+"#reviewBestDiv .sub_gnb_cate li a").eq(0).click();
...
//                        mmain.best.reviewBestSwipe();
                        ...
                        // 판매 베스트
                        $(tabClass+"#sellBestDiv .sub_gnb_cate li a").eq(0).click();
        ...
        bindEvent : function() {
			...
			*/
            ...
            $(tabClass+'#mTab').find('a').bind("click", function(e){
              ...
              //[3411696] '20년상반기 웹앱접근성 개선필요항목(모바일웹) 수정
                $(this).parent().attr('title', '선택됨');
                $(this).parent().siblings('li').attr('title', '');
            ...
            $(tabClass+"#sellBestDiv .sub_gnb_cate li a").bind("click", function() {
...
//                }
            ...
            $(tabClass + "#mSubGnb2").find("li").each(function(idx) {
                $(this).find("a").click(function() {
                	$(document).scrollTop(0);
                    $(tabClass + "#mSubGnb2").find("li").removeClass("on");
                    $(this).parent().addClass("on");
//                    alert(1);
                    ...
                    //페이징 콜러 제거
                    PagingCaller.destroy();
                    $(tabClass+".help_review_wrap").empty();
...
//                    alert(2);
                    ...
                    mmain.best.reviewSubGnbInit();
                    mmain.best.scrollToMenu(idx, 3);
                });
            });

//            $(tabClass+"#reviewBestDiv .sub_gnb_cate li a").bind("click", function() {
//            	
//            	
//            });
...
//              $(".goodsList").removeClass("goodsList");
        ...
        selDefaultCat : function() {
            ...
            */
            ...
            if(dispCatNo == '900000100100002'){
                // 리뷰 베스트
                $(tabClass+"#mSubGnb2 .sub_gnb_cate li").removeClass("on");
                $(tabClass+"#mSubGnb2 .sub_gnb_cate li").eq(0).addClass("on");
                $(tabClass+"#mSubGnb2 .sub_gnb_cate li.on a").click();
...
//                mmain.best.reviewBestSwipe();
                ...
                // 판매 베스트
                $(tabClass+"#mSubGnb .sub_gnb_cate li").removeClass("on");
                $(tabClass+"#mSubGnb .sub_gnb_cate li").eq(0).addClass("on");
                $(tabClass+"#mSubGnb .sub_gnb_cate li a[data-ref-dispCatNo='']").click();
        ...
        getBestPagingAjax : function(startIdx) {
            ...
            */
            ...
            if(dispCatNo == '900000100100002'){
                // 리뷰 베스트
                fltDispCatNo = $(tabClass+"#mSubGnb2 ul li.on a").attr("data-ref-dispCatNo");
                rowsPerPage = 10;
            }
//            var targetUrl = "";
//            if(dispCatNo == '900000100100002'){
//                // 리뷰 베스트
//            	targetUrl = _baseUrl + "main/getBestPagingAjax.do"
//            }else{
//            	targetUrl = _baseUrl + "main/getBestPagingAjax.do"
//            }
            ...
            if (startIdx < 2 && dispCatNo != "900000100100004") {
            	$(tabClass+"#rankBestDiv").hide();
                //연속키 방식
                PagingCaller.init({
                    callback : function(){
                        //코드 완료 후 스토어 정보 조회
            ...
            }else if(dispCatNo == "900000100100004"){
            	mmain.best.getRankBestInit();
            }
        },
        getRankBestInit : function() {
        	/* megabrand
            let tabClass = "#main-swiper-tab3" + " ";
            */
            let tabClass	= "";
            ...
            $(tabClass+"#rankBestDiv").show();


            var url = _baseUrl + "main/getRankBestAjax.do";
            common.Ajax.sendRequest("POST", url, {}, function(res) {
            	$("#rankBestDiv").html(res);
            });
        },
        getBestPagingAjaxCallback : function(res) {
            ...
            */
            ...
            if (res.trim() == '' || PagingCaller.getCurPageIdx() > destroyIdx) {
                //응답 결과가 없을 경우 마지막 목록임- 스크롤 이벤트 제거
                PagingCaller.destroy();
                ...
                if(dispCatNo == '900000100100002'){
                    // 리뷰 베스트인 경우
                	$(tabClass+".help_review_wrap").append(res);
                	
                	 $(".recom_review>ul.inner>li .txt dd").css('word-break' , 'break-all');
                }else{
                    // 판매 베스트인 경우
                    $(tabClass+".mlist2v-best").append(res);
                    $(tabClass+ '#sellBestDiv .goods-list-container').append(res);
                }
                
                mmain.main.setLazyLoad();

                if (PagingCaller.getCurPageIdx() >= destroyIdx) {
                    //마지막 목록임- 스크롤 이벤트 제거
                    PagingCaller.destroy();
                }
            }
...
//              $(".goodsList").removeClass("goodsList");
                ...
                $('.mlist2v-best.prod-list.v2 .prod').each(function (i) {
                    ...
                    var _data_goodsno = _item.find('a').attr('data-ref-goodsno');
                    var goodNm =  _item.find('.prod-info a .prod-name.double-line').text();
                    var dataAttr = _item.find('a').attr("data-attr");

                    if(dataAttr == undefined || dataAttr == "null" || dataAttr == null){
                        var categoryTxt = sessionStorage.getItem("bestCategoryTxt");
                        _item.find('a').attr("data-attr", '랭킹^판매랭킹리스트_' + categoryTxt + '^' + goodNm);
                        _item.find('a').attr('onclick', 'gtm.goods.callGoodsGtmInfo("' + _data_goodsno + '", "", "ee-productClick", "랭킹_판매랭킹리스트_' + categoryTxt + '","' + (i + 1) + '");');
                        _item.find('a.thumb').attr("data-impression", _data_goodsno + '^랭킹_판매랭킹리스트_' + categoryTxt + '^' + (i + 1));
...
//        },
        ...
        reviewSubGnbInit : function() {
        	...
        	console.log("[mmain.best.reviewSubGnbInit]");
            ...
            $(tabClass + "#bestCate2 > li").click(function() {
                if(checkVal == 0) {
                    var count =  $( "#bestCate2 > li" ).index(this) + 1;
                    common.wlog("best_category_tab_p" + count);
                    checkVal ++;
                }
            });
            ...
            $("#bestCate2 li").each(function() {
                if($(this).hasClass('on')) {
                    categoryTxt = $(this).find("a").text();
                }
            });

            $("#reviewBestGrid li a").each(function() {
                var reviewRank = $(this).find('.thum .num').text();
                $(this).attr("data-attr" , '랭킹^리뷰베스트_' + categoryTxt + '^' + reviewRank);

            });


            $(tabClass + "#mSubGnb2 ul li:last-child").css("padding-right", "5px");
            var initSlide = 0;

            if ($(tabClass + "#mSubGnb2 ul li.on").length > 0 ) {
                initSlide = $(tabClass + "#mSubGnb2 ul li.on").index();
            }

            try{
                let tabSwiper = $(tabClass + '#mSubGnb2')[0];
                if(tabSwiper != undefined || tabSwiper != null) {
                    if (tabSwiper.swiper != null) {
                        tabSwiper.swiper.destroy();
                    }
                }
                tabSwiper = new Swiper(tabClass + '#mSubGnb2', {
                    initialSlide: initSlide,
                    slidesPerView: 'auto',
                    paginationClickable: true,
                    spaceBetween: 0,
...
//            mmain.subGnb.mSubGnb_swiper =
            ...
            mmain.best.scrollToMenu(initSlide, gnbTabIdx);
        ...
        scrollToMenu : function(gnbTabIdx, parentGnbTabIdx) {
            ...
            var gnbMenuTag = $(tabClass+'#mSubGnb2').find('ul');
            var gnbMenu = $(tabClass+".sub_gnb_cate.swiper-wrapper");
            //cell width
            var gnbCellWidth = gnbMenuTag.find("li").eq(gnbTabIdx).outerWidth();
            var gnbCellCnt = gnbMenuTag.find("li").length;
...
//                .css('transition-duration','500ms');
      ...
      //리뷰 고도화 2차 S
      //layer 팝업 S *공통으로 사용되고 있을 수 있으니 확인하고 사용 바랍니다.
    ...
    init : function() {
        ...
        */
        ...
        setTimeout(function() {
            ...
            common.bindGoodsListLink();
            //리뉴얼 상품 카드 적용시 삭제 예정
            common.bindGoodsGtmInfo('#mmainNews','신상_기획전상세');
//            $(".goodsList").bind("click", function(e) {
//                e.preventDefault();
//                common.link.moveGoodsDetail($(this).attr("data-ref-goodsNo"), $(this).attr("data-ref-dispCatNo"));
//            });
            //링크 처리
            $("img[data-ref-link-url], video[data-ref-link-url]").bind("click", function() {
...
//            $(".goodsList").removeClass("goodsList");
    ...
    bindWebLog : function (){
        ...
        $(".submain-new .image").each(function(i) {
            ...
            $(this).attr("data-attr", '신상^기획전상세^'+$(this).children().find('.subctit').text()+'^'+(i+1));
        ...
        */
        ...
        PagingCaller.init({
            callback : function(){
                ...
                /* 신상 페이징 2021.09.14일 모바일 상품카드 A/B test때 캐시적용 안함 */
...
mmain.hotdeal = {
    ...
    init : function() {
        ...
        */
        ...
        var sort = $(".ts_sort_area ul li").find('.on').attr("data-ref-sort");
        //최초에는 1페이지를 로딩한 상태이므로 2페이지부터 호출하도록함.
        mmain.hotdeal.getHotdealPagingAjax(startIdx, null, sort);

        //서브 메뉴바 바인드 처리
        mmain.hotdeal.subCtgBindEvent();

        //정렬 조건 바인드 처리
        mmain.hotdeal.prdSortBindEvent();
...
//            $(".goodsList").removeClass("goodsList");
    ...
    bindWebLog : function (){
        ...
        */
        ...
        var wlogID = "";
        var mmainHotdeal_sortTxt = document.getElementById('todaySpecial-sorting').querySelector('.active').textContent;

        //AB Test용 웹로그 추가
        $(tabClass+"#mmainSpecialHotdeal .prod").each(function(i) {
            if( common.isEmpty($(this).attr("data-wlog-id") ) ) {
                wlogID = 'todaySpecialGoods';
                var _data_goodsno = $(this).find('a').attr('data-ref-goodsno');
                $(this).find('a').attr('onclick', 'javascript:common.wlog("'+wlogID+'");gtm.goods.callGoodsGtmInfo("'+_data_goodsno+'", "", "ee-productClick", "오특_오특상품리스트_스페셜_A", "'+(i+1)+'");');
            }

        });

        //웹로그ID 생성
        $(tabClass+"#mmainHotdeal .prod").each(function(i) {
            if( common.isEmpty($(this).attr("data-wlog-id") ) ) {
                wlogID = 'todayDailyGoods';
                var _data_goodsno = $(this).find('a').attr('data-ref-goodsno');
                $(this).find('a').attr('onclick', 'javascript:common.wlog("'+wlogID+'");gtm.goods.callGoodsGtmInfo("'+_data_goodsno+'", "", "ee-productClick", "오특_오특상품리스트_A_'+mmainHotdeal_sortTxt+'", "'+(i+1)+'");');
            }
        });
    },

    getHotdealPagingAjax : function(startIdx, dispCatNo, sort) {
        ...
        PagingCaller.init({
            callback : function(){
                ...
                var param = {
                    ...
                    fltCondition : "02",
                    fltDispCatNo : (dispCatNo == undefined || dispCatNo == null) ? "" : dispCatNo,
                    prdSort : (sort == undefined || sort == null) ? "rank" : sort,
                };
    ...
    prdSortBindEvent : function() {
        ...
        */
        ...
        $(tabClass+".ts_sort_area").find("li").each(function() {
            $(this).find("a").click(function() {
                $(document).scrollTop(0);

                var sort = $(this).attr("data-ref-sort");
                ...
                mmain.hotdeal.getHotdealPagingAjax(0, null, sort);
...
//                        mmain.menu.moveSubMenu(idx,  $("#mGnb").find("li").eq(idx).attr("data-ref-url"));
    ...
    _callback_getEventListAjax : function(strData) {
        ...
        dispObj.empty();
        mmain.event.makeEventList(strData.evtType, dispObj,strData.eventList);
...
//        sessionStorage.setItem("saved_slide_html", $('#mContainer').html());
    ...
    makeEventList :  function(evtType, dispObj,eventList){
        if(eventList.length <= 0){
            ...
            var eventTypeFromEvtType = evtType == "20" ? "모든회원" : "구매회원";
            ...
            $.each(eventList, function(index, element){
                ...
                var $aTag = $("<a>").attr({
                    "href" :"javascript:;",
                    "data-attr" : "이벤트^이벤트_"+ eventTypeFromEvtType + "^" + element.bnrMainTxt,
                });
    ...
    makeBeautyList :  function(dispObj,beautyList){
        ...
        if(beautyList.length <= 0){
            ...
            $.each(beautyList, function(index, element){
                ...
                var $aTag = $("<a>").attr({
                    "href" :"javascript:;",
                    "data-attr" : "이벤트^이벤트_올영체험단^"+ element.custFvrAplyTgtVal,
                });
                $li.append($aTag);
    ...
    _callback_getOllyoungApplyDetailAjax : function(strData) {
        if(typeof(strData) === 'object'){
            ...
            if(ret == '0'){
                ...
                if(ret == '033') {
                    // [3537814] 올영체험단 개편으로 인한 팝업 내용수정 요청 件 > 행사 시작일 체크 알럿 수정
                    var tmpStrtDate = detail.startDate;
                    var strtArr = tmpStrtDate.split("/");
                    alert(strtArr[1]+'월 '+strtArr[2]+'일 오전 10시부터 신청 가능합니다. 리뷰 활동 활발히 하여 랭킹을 올려보세요!');
                }else if(ret == '-1'){
                    alert('이미 다른 상품의 올영체험단을 신청하셨습니다.\n동일기간 내에는 1개의 상품만 신청 가능합니다.');
                }else{
                    alert('일시적으로 오류가 발생하였습니다. 다시 시도해주세요.');
                }
                common.popFullClose();
            }
        }
    ...
    _callback_getOllyoungApplyRegAjax : function(strData) {
        if(typeof(strData) === 'object'){
            ...
            if(ollyoungList != null){
                var sDtime = ollyoungList[0].startDate;
                var sArr = sDtime.split("/");
                var sDate = new Date(sArr[0], sArr[1]-1, sArr[2]);
                var eDtime = ollyoungList[0].endDate;
                var eArr = eDtime.split('/');
                var eDate = new Date(eArr[0], eArr[1]-1, eArr[2]);
                var dDate = sDate.addDate(9);
                var tmpSDate = "";
                if (dDate > eDate) {
                    tmpSDate = eDate.format('yyyy/MM/dd');
                } else {
                    tmpSDate = dDate.format('yyyy/MM/dd');
                }
                var delArr = tmpSDate.split("/"); // 배송일정
                if( ret == 0){
                    alert('축하합니다! 신청 완료됐습니다.\n아래 내용 꼭 확인해주세요!\n\n- 배송 일정 : '+delArr[1]+'월 '+delArr[2]+'일부터\n- 리뷰 마감 : '+eArr[1]+'월 '+eArr[2]+'일까지');
                    common.popFullClose();
//                    mmain.event.getOllyoungListAjax(null);
...
//                	common.link.commonMoveUrl("main/main.do#5_2");
                    ...
                    alert('죄송합니다. 해당 상품의 올영체험단 신청이 마감되었습니다.\n 다른 상품으로 신청해주세요.');
                }else if(ret == '-1'){
                    alert('이미 다른 상품의 올영체험단을 신청하셨습니다.\n동일기간 내에는 1개의 상품만 신청 가능합니다.');
                }else if(ret == '098'){
                    if(!confirm("해당 서비스는 로그인이 필요합니다. 로그인 페이지로 이동하시겠습니까?")){
...
//        sessionStorage.setItem("saved_slide_html", $('#mContainer').html());
    ...
    if(totalCount <= 0){
        ...
        if(pageIdx == 1){
            ...
            $term.append($term1);
            // [3537814] 올영체험단 개편으로 인한 팝업 내용수정 요청 件 > 신청기간 문구 수정
            var tmpStrtDate = ollyoungList[0].startDate;
            var strtArr = tmpStrtDate.split("/");
            // [3537814] 올영체험단 개편으로 인한 팝업 내용수정 요청 件 > 신청기간 종료일자 시작일로부터 + 7일
            var endDate = new Date(strtArr[0], strtArr[1]-1, strtArr[2]);
            var tmpEndDate =  endDate.addDate(7).format('yyyy/MM/dd');
            var endArr = tmpEndDate.split("/");
            var $em1 = $("<em>").text(strtArr[0].slice(-2)+"."+strtArr[1]+"."+strtArr[2] +" ~ "+ endArr[0].slice(-2)+"."+endArr[1]+"."+endArr[2]);
            $term1.append($em1);
            $term1.append(' 입니다.');
            $term.append($term1); 
        ...
        if(aplyDdl >= maxCnt){
            ...
            $.each(ollyoungList, function(index, element){
                if(sEvtKeyword == 'A'){
                    ...
                    var nowArr = tmpNowDate.split("/");
                    var nowDate = new Date(nowArr[0], nowArr[1], nowArr[2]);
                    // [3537814] 올영체험단 개편으로 인한 팝업 내용수정 요청 件 > 행사 시작일 체크시 시간까지
                    var nowDatetime = new Date();
                    var strtDatetime = new Date(element.startDate+" "+element.strtDtime);
                    var $li = $("<li>");
                    $ulEvent.append($li);
                    ...
                    var $img = $("<img>").attr("src",url.join("")).attr('onerror',"common.errorImg(this);").attr("alt","이벤트 이미지").attr("data-attr", "이벤트^이벤트_올영체험단^" + element.custFvrAplyTgtVal);
                    $thum.append($img);
...
//                        var $span1 = $("<span>").addClass("colog fr").text(element.regNum);
                    ...
                    if(parseInt(element.regNum) >= parseInt(element.rcrtCnt) || endDate.getTime() < nowDate.getTime()){
                        ...
                        // [3537814] 올영체험단 개편으로 인한 팝업 내용수정 요청 件 > 행사 시작일 체크 알럿 수정
                        if(nowDatetime < strtDatetime){
                            $dis.addClass("btnApplyLayer").attr('onclick','alert("'+strtArr[1]+'월 '+strtArr[2]+'일 오전 10시부터 신청 가능합니다. 리뷰 활동 활발히 하여 랭킹을 올려보세요!");').text("신청하기");
                        }else{
...
//             $dl1.append($span2);
               ...
               var $btn1 = $("<button>").addClass("btnacc").attr("type","button").text("개인정보 수집 이용 동의 열기");
               ...
               var $btn2 = $("<button>").addClass("btnacc").attr("type","button").text("개인정보 취급 위탁 동의 열기");
...
//             $($ul3).append($li11);
               ...
               //[3411780] '20년상반기 웹앱접근성 개선필요항목 (Android)
               $('.btnacc').on('click', function(){
                    var _this= $(this);
                    var _thisHead = _this.closest('.head_box');
                    if(_thisHead.hasClass('on') == true){
                    	var btnaccStr = _this.text().replace('열기','닫기');
                    	_this.html(btnaccStr);
                        _thisHead.removeClass('on');
                    }else{
                    	var btnaccStr = _this.text().replace('닫기','열기');
                    	_this.html(btnaccStr);
...
//            sessionStorage.setItem("saved_slide_html", $('#mContainer').html());
    ...
    setPopup : function(obj){
        ...
        localStorage.setItem('lyScrollSet',$(document).scrollTop());
    ...
    popLayerIdOpen : function(id) {
        common.popLayerOpen(id);
    },
    ...
    popLayerIdClose : function(id) {

        common.popLayerClose(id);
...
$.namespace("mmain.foryou");
mmain.foryou = {
	init : function(startIdx){
		var url = _baseUrl + "mycuration/getCurationForYouAjax.do"
		common.Ajax.sendRequest("POST", url, {}, function(res) {
			/* megabrand
            let tabClass = "#main-swiper-tab6" + " ";
            */
            let tabClass 	= "";
            var date 		= new Date();
            var today 		= "" + date.getFullYear() + ("0" + (1 + date.getMonth())).slice(-2) + ("0" + date.getDate()).slice(-2)    + ("0" + date.getHours()).slice(-2);
            if (today >= '2021100709' && today <= '2021100823') {
                tabClass = "#main-swiper-tab7" + " ";
            } else {
                tabClass = "#main-swiper-tab6" + " ";
            }
			
			$(document).scrollTop(0);
			$(tabClass).parents(".swiper-wrapper:eq(0)").height(1000);
			$(tabClass).empty();
			$(tabClass).html(res);
		});
	}
